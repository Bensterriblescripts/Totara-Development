...
class persons extends handler {
    ...
    */
    ...
    private function update_person(stdClass $person, $conditions = array()) {
        ...
        $supervisorchanges = false;
        $newassessorgrouprecord = false;
        $newsupervisorrecord = false;
        ...
        //Determine supervisor field changes
        if ($DB->record_exists('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9']) && !empty($person->trainingsupervisorid)) {
            $supervisorcurrent = $DB->get_record('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9']);
            if ($person->trainingsupervisorid != $supervisorcurrent->data) {
                $supervisorchanges = true;
            }
            $haschanges = true;
        }
        else if (!$DB->record_exists('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9']) && !empty($person->trainingsupervisorid)){
            $newsupervisorrecord = true;
            $haschanges = true;
        }
        ...
        if ($supervisorchanges) {
            ...
            $userdata = $DB->get_record('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9']);
            $userdata->data = $person->trainingsupervisorid;
            $DB->update_record('user_info_data', $userdata);

            $supervisor = $DB->get_record('user', ['idnumber' => $person->trainingsupervisorid]);
            echo 'User assigned supervisor with id ' . $person->trainingsupervisorid . ' and email ' . $supervisor->email;
        }
        //Create new supervisor record
        if ($newsupervisorrecord){
            $infodatamappings = array(
                'userid'            => $mdluser->id,
                'fieldid'           => '9',
                'data'              => $person->trainingsupervisorid,
                'dataformat'        => '0'
            );
            $DB->insert_record('user_info_data', $infodatamappings);
        ...
        //If the user is a supervisor, set their supervisor field to their own id to auto-group learner-supervisor
        if ($userissupervisor){
            if ($DB->record_exists('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9'])) {
                $selfid = $DB->get_record('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9']);
                $selfid->data = $mdluser->idnumber;
                $DB->update_record('user_info_data', $selfid);

                echo 'Updated supervisor id with itself for user: ' . $mdluser->id;
            }
            else if (!$DB->record_exists('user_info_data', ['userid' => $mdluser->id, 'fieldid' => '9'])) {
                $idnumbermappings = array (
                    'userid'        => $mdluser->id,
                    'fieldid'       => '9',
                    'data'          => $mdluser->idnumber,
                    'dataformat'    => '0'
                );
                $DB->insert_record('user_info_data', $idnumbermappings);
            }
        }
