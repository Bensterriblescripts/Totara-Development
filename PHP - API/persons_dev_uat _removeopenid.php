...
class persons extends handler {
    ...
    */
    ...
    private function create_person(stdClass $person) {
        ...
        // If the user has something other than oidc, we can't continue. This shouldn't happen.
        if ($person->auth != 'oidc') {
            // Generate an error message.
            $errormessage = $this->get_string(
                'process',
                "Can't add the person with id {$person->id} using email address {$person->email}!".
                " The user does not have a valid authentication."
            );
     ...
     * do all of its user creation stuff.
     * 
     * TODO - Set up new user's profile fields in this function too
     *
    ...
    private function add_person(stdClass $itomicperson, stdClass $person) {
        ...
        if (isset($person->auth)) {
            //This should always be forced to open id connect 'oidc'
    ...
    */
    ...
    private function construct_person(stdClass $person) {
        ...
        //Set the authentication to the value provided by CRM - This should only be oidc
        if ($person->auth) {
